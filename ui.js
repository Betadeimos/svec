...
const App = () => {
  ...
  useInput((input, key) => {
    ...
    // Enter (return) now acts exactly like Tab as requested
    if (key.tab || input === '\t' || key.return) {
      if (key.shift) {
        if (activeField > 0) {
           setActiveField(prev => {
             // If we're at Output(2) but showOutput is false, skip to 0
             if (prev === 2 && !showOutput) return 0;
             if (prev === 2) return 1;
             return 0;
           });
        } else {
           setActiveTab((prev) => (prev === 0 ? tabs.length - 1 : prev - 1));
           setActiveField(showOutput ? 2 : 1);
        }
      } else {
        if (activeField < (showOutput ? 2 : 1)) {
  ...
  return React.createElement(Box, { 
      ...
      alignItems: "center", 
      justifyContent: "center", // Changed to true vertical center
      overflow: "hidden" 
    },
    // LOGO & SUBTITLE 
    (showLogo || showSubtitle) && React.createElement(Box, { 
        ...
        alignItems: "center",
        width: "100%",
        marginBottom: 1 // Adding an explicit spacing gap back properly here
      },
      ...
      showSubtitle && React.createElement(Box, { marginTop: 0 },
        React.createElement(Text, { color: "gray" }, "SVEC • Simplest Video Editor CLI • v1.0.9")
      )
    ),
    ...
    React.createElement(Box, { 
      ...
      height: Math.max(15, Math.min(26, size.rows - (showLogo ? 10 : 2))), // Constrained dynamic height prevents infinite stretching
      borderStyle: "round", 
      borderColor: "cyan", 
      flexDirection: "column", 
      paddingX: 1, 
      ...
      React.createElement(Box, { width: "100%", flexGrow: 1, flexDirection: "column", overflow: "hidden" },
        activeTab === 0 ? 
          React.createElement(Box, { width: "100%", flexDirection: "column", flexGrow: 1, overflow: "hidden" },
            ...
            React.createElement(Box, { 
                ...
                paddingX: 1, 
                marginBottom: 0, // Tightly compressed
                flexShrink: 0,
                overflow: "hidden"
              },
            ...
            showOutput && React.createElement(Box, { 
                width: "100%",
                marginTop: 0, // Compressed spacing
                borderStyle: "single", 
        ...
        : 
          React.createElement(Box, { width: "100%", flexGrow: 1, alignItems: "center", justifyContent: "center" },
            React.createElement(Text, { color: "gray" }, `Settings for ${tabs[activeTab]} placeholder...`)
          )
      ),
